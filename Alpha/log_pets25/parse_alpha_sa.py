...
DIR_SA = DIR_LOG + "alpha_sa/"

def parse_file(n_clients, n_inputs):
    output_line = f"{n_clients},{n_inputs}"
    time_logs = []
    log_file_path = f"{DIR_SA}/sa_{n_clients}_{n_inputs}.log"
    ...
    time = {}
    data = {}
    for key in [f"Time{i}" for i in range(1,6)]:
        time[key] = []
        data[key] = []
...
output_file = open(f"{DIR_LOG}/alpha_sa.csv", "w")
output_file.write("N_Clients,N_Inputs,Total_Time,Total_Sent,B2A_Time,B2A_Sent,L2_Time,L2_Sent,Aggr_Time,Aggr_Sent,Veri_Time,Veri_Sent\n")
