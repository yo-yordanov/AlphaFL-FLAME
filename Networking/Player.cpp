        ...
        const vector<int>& ports, const string& id_base, ServerSocket& server)
{
    ...
    for (int i = 0; i < nplayers + 1; i++) {
        ...
        tv.tv_sec = 1800;
        tv.tv_usec = 0;
        int fl = setsockopt(tmp_ss[i], SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(struct timeval));
        if (fl<0) { error("set_up_socket:setsockopt");  }

        fl = setsockopt(tmp_ss[i], SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv));
        if (fl<0) { error("set_tcp_keepalive:setsockopt(SOL_SOCKET, SO_SNDTIMEO)");  }

        int enable_keepalive = 1;
        fl = setsockopt(tmp_ss[i], SOL_SOCKET, SO_KEEPALIVE,(char *)&enable_keepalive, sizeof(enable_keepalive));
...
#endif
  comm_stats["Sending directly"].add(o);
  auto tmp_timer = Timer();
  TimeScope ts(tmp_timer);
